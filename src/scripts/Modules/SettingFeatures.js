async function CreateSettingFeatures() {
	...
	await CreateFeatures("Check", {
		id: "AutoPIP",
		description:
			"Auto Pictue In Pictue mode<br>(Pls click anywhere In page after you back to page)<br>(Security problem) (I do my best T_T)",
		Default: true,
	});
	...
	await CreateFeatures("CombinationFunc", {
		id: "SetMediaSpace",
		multi_id: ["ControlUnderVDO", "MediaSpace"],
	...
	await CreateFeatures("CombinationFunc", {
		...
		multi_id: ["ControlUnderVDO", "AutohideBar"],
		CSSFunc: async function () {
	...
	await CreateFeatures("NumberSlide", {
		id: "LazyAmount",
		description:
			"CoolDown Every ... pixel<br>(Check black bars)<br>(less for more performance also more drop frame)<br>(-1 is none)",
		Default: 50,
		min: -1,
		max: 200,
	});
	...
	await CreateFeatures("CombinationFunc", {
		id: "TopIcon",
		multi_id: ["ReplaceYT", "ReplaceYTURL"],
		CSSFunc: async function () {
	...
	await CreateFeatures("Select", {
		...
		ItemsCSS: {
			...
			Sha: JSON.stringify([
				`box-shadow: 0px 0px var(--border-width) var(--border-color) !important;`,
				`box-shadow: var(--player-bg-border-width) var(--player-bg-border-width) 10px var(--border-color) !important;`,
			]),
			None: JSON.stringify([
	...
	await CreateFeatures("JustText", {
		innerHTML: "Some features need to reload website!",
		align: "left",
	});
	...
	await CreateFeatures("Check", {
		id: "NewVDOanima",
		description: "Enable new video animation (play , stop , change volume, forward , backward)",
		Default: true,
		AutoUpdate: true,
		new: true,
	});
