...
export function generateSelectors(element: HTMLElement): string[] {
	...
	if (parent && parent !== document.body) {
		...
		const parentDesc = parent.id ? `#${parent.id}` : 
						 (parentClasses.length > 0 ? `${parent.tagName.toLowerCase()}.${parentClasses[0]}` : parent.tagName.toLowerCase());
	...
	return Array.from(new Set(selectors.filter((s) => {
		try {
			if (!s || s === "") return false;
